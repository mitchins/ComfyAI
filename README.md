...
cd ComfyAI
pip install comfyai           # nodes only
pip install 'comfyai[onnx]'   # add ONNX server
```
...
Use the `comfyai-onnx-server` command (or `python -m apps.onnx_server`) for a lightweight OpenAI compatible endpoint. Install the optional dependencies first:
