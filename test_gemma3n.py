...
# Only allow CUDA if explicitly enabled via environment
ALLOW_CUDA = os.environ.get("ALLOW_CUDA", "").lower() in ("1", "true", "yes")

# Verify CUDA provider availability if explicitly requested
available_eps = onnxruntime.get_available_providers()
if ALLOW_CUDA and "CUDAExecutionProvider" not in available_eps:
    raise RuntimeError(
        "ALLOW_CUDA is set but CUDAExecutionProvider is not available; aborting."
    )

def make_session(model_path):
    """
    Create an ONNX Runtime session with a single provider:
    - CUDAExecutionProvider if ALLOW_CUDA is True (no fallback).
    - CPUExecutionProvider otherwise.
    Raises on failure to load the requested provider.
    """
    provider = "CUDAExecutionProvider" if ALLOW_CUDA else "CPUExecutionProvider"
    try:
        ...
        raise RuntimeError(
            f"Failed to load {model_path} with provider {provider}: {e}"
        )
    print(f"Loaded {model_path} with {provider}")
    return session
